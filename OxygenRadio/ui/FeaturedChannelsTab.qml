    ...
    function get_stations() {
        ...
        for (i = 0; i < xmlModel.count; i++) {
            ...
            if (featured == true) {
                ...
                featuredModel.append({"id": id, "title": title, "description": description, "image": image, "playlist_url": playlist_url, "listen_url": listen_url, "uid": uid, "stars": stars, "featured": featured, "is_favorited": is_favorited});
            }
        }
        ...
        var n;
        var j;
        for (n=0; n < featuredModel.count; n++) {
            for (j=n+1; j < featuredModel.count; j++)
            {
                if (featuredModel.get(n).stars < featuredModel.get(j).stars) {
                    featuredModel.move(j, n, 1);
                    n=0;
                }
            }
        }
                    ...
                    Action {
                        ...
                        onTriggered: {
                            if (is_favorited) {
                                ...
                                featuredModel.setProperty(index, "is_favorited", false)
                                iconColor = UbuntuColors.lightGrey
                                favoritesPage.get_stations()
                            } else {
                                Scripts.fav_channel(id, 1)
                                featuredModel.setProperty(index, "is_favorited", true)
